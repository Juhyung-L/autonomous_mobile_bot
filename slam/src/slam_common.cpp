...
#include "slam/serialization.hpp"
...
/*****************************************************************************/
Slam::Slam()
: Slam(rclcpp::NodeOptions())
/*****************************************************************************/
{
}

/*****************************************************************************/
Slam::Slam(rclcpp::NodeOptions options)
: nav2_util::LifecycleNode("slam", "", options),
  solver_loader_("slam", "karto::ScanSolver"),
...
nav2_util::CallbackReturn 
Slam::on_shutdown(const rclcpp_lifecycle::State & state)
{
  RCLCPP_INFO(get_logger(), "Shutting down slam...");
  return nav2_util::CallbackReturn::SUCCESS;
}
...
/*****************************************************************************/
{
  ...
  ssMap_ = this->create_service<nav_msgs::srv::GetMap>("slam/dynamic_map",
      std::bind(&Slam::mapCallback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ssPauseMeasurements_ = this->create_service<slam::srv::Pause>(
    "slam/pause_new_measurements",
    std::bind(&Slam::pauseNewMeasurementsCallback,
    this, std::placeholders::_1,
    std::placeholders::_2, std::placeholders::_3));
  ssSerialize_ = this->create_service<slam::srv::SerializePoseGraph>(
    "slam/serialize_map",
    std::bind(&Slam::serializePoseGraphCallback, this,
    std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  ssDesserialize_ = this->create_service<slam::srv::DeserializePoseGraph>(
    "slam/deserialize_map",
    std::bind(&Slam::deserializePoseGraphCallback, this,
    std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  ...
  scan_filter_->registerCallback(
    std::bind(&Slam::laserCallback, this, std::placeholders::_1));
}

/*****************************************************************************/
void Slam::publishTransformLoop(
...
/*****************************************************************************/
void Slam::publishVisualizations()
/*****************************************************************************/
{
...
/*****************************************************************************/
void Slam::loadPoseGraphByParams()
/*****************************************************************************/
{
  std::string filename;
  ...
  if (shouldStartWithPoseGraph(filename, pose, dock)) {
    std::shared_ptr<slam::srv::DeserializePoseGraph::Request> req =
      std::make_shared<slam::srv::DeserializePoseGraph::Request>();
    std::shared_ptr<slam::srv::DeserializePoseGraph::Response> resp =
      std::make_shared<slam::srv::DeserializePoseGraph::Response>();
    req->initial_pose = pose;
    req->filename = filename;
    if (dock) {
      req->match_type =
        slam::srv::DeserializePoseGraph::Request::START_AT_FIRST_NODE;
    } else {
      req->match_type =
        slam::srv::DeserializePoseGraph::Request::START_AT_GIVEN_POSE;
    }
...
/*****************************************************************************/
bool Slam::shouldStartWithPoseGraph(
  std::string & filename,
...
/*****************************************************************************/
{
  ...
  if (!filename.empty()) {
    ...
    if (map_start_pose.get_type() != rclcpp::ParameterType::PARAMETER_NOT_SET) {
      ...
      start_at_dock = false;
      if (read_pose.size() != 3) {
        RCLCPP_ERROR(get_logger(), "LocalizationSlam: Incorrect "
      ...
      RCLCPP_ERROR(get_logger(), "LocalizationSlam: Map starting "
...
/*****************************************************************************/
LaserRangeFinder * Slam::getLaser(
  const
...
/*****************************************************************************/
tf2::Stamped<tf2::Transform> Slam::setTransformFromPoses(
...
/*****************************************************************************/
LocalizedRangeScan * Slam::getLocalizedRangeScan(
...
/*****************************************************************************/
LocalizedRangeScan * Slam::addScan(
  LaserRangeFinder * laser,
...
/*****************************************************************************/
{
  ...
  if (processor_type_ == PROCESS) {
    ...
    RCLCPP_FATAL(get_logger(),
      "Slam: No valid processor type set! Exiting.");
    exit(-1);
  }
...
/*****************************************************************************/
bool Slam::pauseNewMeasurementsCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
/*****************************************************************************/
{
  ...
  RCLCPP_INFO(get_logger(), "Slam: Toggled to %s",
...
/*****************************************************************************/
bool Slam::isPaused(const PausedApplication & app)
/*****************************************************************************/
{
...
/*****************************************************************************/
bool Slam::serializePoseGraphCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
/*****************************************************************************/
void Slam::loadSerializedPoseGraph(
...
/*****************************************************************************/
bool Slam::deserializePoseGraphCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<slam::srv::DeserializePoseGraph::Request> req,
  std::shared_ptr<slam::srv::DeserializePoseGraph::Response> resp)
/*****************************************************************************/
{
  if (req->match_type == slam::srv::DeserializePoseGraph::Request::UNSET) {
